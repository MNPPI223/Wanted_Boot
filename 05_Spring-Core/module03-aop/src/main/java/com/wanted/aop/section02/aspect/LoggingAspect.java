...
public class LoggingAspect {
    ...
    /* comment.
        ...
        - (String, ..) : 첫 번째 String 이어야 하며, 나머지는 상관 없음.
     */

    @Before("execution(* com.wanted.aop.section02.MemberService.*(..))")
    public void logBefore(JoinPoint joinPoint) {

        // JoinPoint 는 Advice 메서드가 적용되는 지점에 대한 정보를 제공하는
        // 객체이다!!!!!!!!!!!
    ...
    public void logAfter(JoinPoint joinPoint) {
        String methodName = joinPoint.getSignature().getName();
        System.out.println("[After 로그] " + methodName + "메서드 실행 종료!");
    }
    ...
    /* comment.
        ...
        - 예외 흐름 복구 로직 등에 사용된다. -> 할 수 있긴하지만, 더 좋은 예외처리로 하는 것이 좋다.
     */
    @AfterThrowing(
