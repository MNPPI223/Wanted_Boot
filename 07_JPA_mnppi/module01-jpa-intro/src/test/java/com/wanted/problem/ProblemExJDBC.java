...
public class ProblemExJDBC {
    ...
    void testDirectSQL() throws SQLException {
        ...
        /* comment. Test 클래스는 검증을 위한 클래스이다.
            Assertions 클래스는 Test 를 위한 검증 메서드를 제공해준다.
         */
        // menuList 가 NotNull 이면 test 통과
        // 그렇지 않으면 test 불통과 ( Null 이면 통과 / NotNull 이면 불통과 )
...
//        }
    ...
    /* comment.
        ...
        이렇게 요구가 빠귀게 되면 데이터베이스 DDL 구문 수정해야하며,
        SQL 구문 수정, Application 코드 수정이 필요하다.
     */

    /* comment.
        3. 패러다임 불일치 문제(객체지향 관점에서 벗어난다.)
        - SQL 관점에서 1:N -> 엔터티 간 연관성을 FK 로 가지고 있는다.
    ...
    void equalsTwoObject() throws SQLException {
        ...
        if(rset.next()) {
            ...
            menu1.setMenuName(rset.getString("MENU_CODE"));
        }
        ...
        if(rset2.next()) {
            ...
            menu2.setMenuName(rset2.getString("MENU_CODE"));
        }
        ...
        /* comment.
            중요!!!
            하나의 DB 에서 동일한 행에 해당되는 데이터를 꺼냈는데
            각기 다른 객체에 담았을 때 발생하는 동일성 문제
