...
public class AsyncEventController {
    ...
    /* comment.
           CompletableFuture 변환형은 비동기 작업의 결과/에외를 나중에
           이어서 처리할 수 있게 된다.
        */

    @PostMapping("/enrollments/{enrollmentId}/completion-summary")
    public CompletionSummaryResponse completionSummary(@PathVariable Long enrollmentId) {
        return asyncEventService.requestCompletionSummary(enrollmentId);
    }

    /* comment.
        메인흐름에서 비동기 작업의 결과가 응답 시 꼭 필요하다면, join() 으로
        기다린 뒤 응답할 수 있다.
        단, join() 을 쓰게 되면 비동기 작업 수행 시간만큼 응답 시간이 길어진다.
     */

    @PostMapping("/enrollments/{enrollmentId}/completion-summary/wait")
    public CompletionSummaryResponse completionSummaryWait(@PathVariable Long enrollmentId) {
