...
public class AsyncConfig implements AsyncConfigurer {
    ...
    public Executor classTaskExecutor(AsyncProperties properties) {
       // 비동기 메서드를 실행하는 워커 스레드 객체
        ...
        // 위까지는 yml 에 작성한 변수 등록을 해둔 상태이다.
        // APP 종료 시 이미 제출 된 비동기 작업이 끊키지 않도록 하는 설정
        executor.setWaitForTasksToCompleteOnShutdown(true);
        // 10초간 APP 이 종려되어도 비동기 작업을 기다려준다.
    ...
    private static class LoggingAsyncExceptionHandler implements AsyncUncaughtExceptionHandler{
        ...
        @Override
        public void handleUncaughtException(Throwable ex, Method mehtod, Object... params) {

            log.error("[비동기 전용 예외 처리기] method = {}, params = {}, message = {}",
                    mehtod.getName(), Arrays.toString(params), ex.getMessage());
