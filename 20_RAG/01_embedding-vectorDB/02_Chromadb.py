"""
[목표]
1. VectorDB 가 왜 필요한가를 이해한다.

[필요성]
- LMS 강좌 50개를 매번 요청 시마다 Embedding 을 하게 되면
...
# Python 3.9는 `str | None` 문법(3.10+)을 지원하지 않아 Optional을 사용한다.
from typing import Optional

import chromadb
# 문서가 들어오면 지정한 모델로 임베ㅐ딩 해라 라는 것을 명령한다.
from chromadb.utils import embedding_functions

# STEP 1. 사용모델, DB 경로, Table 명 설정
...
# MySQL 에서 DDL 구문 (테이블을 생성하는 구문이라고 생각한다.)
...
# STEP 4. 의미 기반 검색 - 자연어로 강좌 찾기
def search_courses(query: str, top_k: int = 3, category: Optional[str] = None):
    """자연어 질의로 강좌를 검색한다.
...
if __name__ == "__main__":
    ...
    # search_courses("하고 싶은 질문 남기기")
    # where 필터에 조건들을 더 추가해서 중급 이상만 추천 될 수 있도록 변경해보기