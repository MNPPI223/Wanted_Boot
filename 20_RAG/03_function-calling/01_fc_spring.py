...
RAG 와는 조금 다른 점
* RAG : "출석 인정 기준이 뭐야?" 라고 물어보면 답은 VectorDB 에 넣어둔 규정.pdf 파일에 데이터가 존재한다.

* FC : "내가 듣고 있는 1번 강의의 진도율은 몇 % 야?" 라고 물어보면 답은 MySQL 수강 이력 DB 에 데이터가 존재한다.
...
# Fast API 에서 역으로 Http 모듈을 보낼 준비
import httpx
...
# 타임아웃 관련 설정
# FastAPI -> Spring 요청 시 Spring 이 응답을 주지 않는다면?
...
# 통합 핸들러 이기 때문에 URL 만 전달할 수도 있고, 필요하면 파라미터가 올 수도 있다.
...
def _spring_get(path:str, params : dict | None = None) -> dict :
    ...
    try : 
        ...
        return {"error" : f"LMS 학사 정보 조회 실패!! (HTTP {e.response.status_code})", "detail" : e.response.text[:200]}
...
if __name__ == "__main__":
    print("=== [0] Spring 호출 테스트 중... ===")
    print(get_my_courses(1))
    print("=================================")
    config = types.GenerateContentConfig(
        tools=[get_my_courses, get_assignments_due, get_my_summary],
        system_instruction=(
            "너는 LMS 학습 도움형 챗봇이다"
            " 현재 로그인한 학생의 ID 는 1이다."
            "학생의 개인 데이터가 필요하면 반드시 함수로 조회하고,"
            "조회 결과에 없는 내용은 '몰라' 라고 대답해라"
            "또한 함수가 error 를 반환한다면 '공습경보! 공습경보!' 라고 말하면서 에러 상황을 정중히 설명해라"
            "절대 실수하지 말 것"
        ),
        temperature=0.2
    )
