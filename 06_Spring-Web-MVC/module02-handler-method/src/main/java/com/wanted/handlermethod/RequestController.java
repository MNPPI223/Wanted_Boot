...
public class RequestController {
    ...
    public String registMenu(Model model, HttpServletRequest request) {
        ...
        String message = menuName + " 을(를) 신규 메뉴 목록" + categoryCode + "번 카타고리에 "
                + menuPrice + " 원 으로 등록 성공했습니다!";
    ...
    /* comment.
        ...
        🚨주의!! view 의 name 속성과 일치하게 작성해야 400 Error 가 발생이 안 되며,
        만약 name 속성과 일치하고 싶지 않으면, 이름을 명시해야한다.
     */
    ...
    /* comment.
        위쪽에서 @RequestParam 요청 시 값을 받아오게 되면
        나중에 전달받을 값이 많아지는 경우에 관리해야 할 변수, 키 값이 많아진다.
        @ModelAttribute 는 클래스 자료형을 활용하여, 여러 값을 한 번에
        받아올 수 있는 기능을 제공한다.
        Model 객체에 addAtribute 를 하지 않아도, 네이밍 규칙에 의해
        사용할 수 있다. ex) @ModelAtribute("menu") -> view 에서 menu 이름으로 값 사용
        ex) @ModelAttribute -> view 에서 menuDTO 이름으로 값을 사용
     */

    @PostMapping("search")
    public String serachMenu(@ModelAttribute("menu") MenuDTO menu) {
    ...
    /* comment.
        @SessionAttribute 를 이용해서 session 에 값 담기
        @SessionAttribute -> 클래스 레벨에 작성한다.
        session 에 담을 값을 설정해두면, Model 영역에
        해당하는 Key로 값이 추가되는 경우, 자동으로 session에 등록해준다.
     */
    ...
    /* comment.
        ...
        SessionStatus 객체의 setComplete() 메소드를 사용해야
        만료시킬 수 있다.
     */
